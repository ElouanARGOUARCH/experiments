...
number_to_switch = int(samples.shape[0]/10)
...
genclass = GenerativeClassifier(train_samples, train_labels, [64,64,64,64], K=6)
...
torch.save(genclass, 'FashionMNIST_models/genclass_0.pt')
for t in range(20):
    ...
    genclass = GenerativeClassifier(train_samples, train_labels, [64,64,64,64], K=6)
    current_train_accuracy, current_test_accuracy = genclass.train(100,5000,verbose=True, lr=5e-3, test_samples = test_samples, test_labels = test_labels, trace_accuracy = True)
    torch.save(genclass, 'FashionMNIST_models/genclass' + str(t+1)+'.pt')
