...
class GenerativeClassifierSemiSupervised(torch.nn.Module):
    ...
    def train(self, epochs, batch_size, train_samples, train_prior_probs, train_labels, unlabeled_samples,
              unlabeled_prior_probs, unlabeled_labels,
              test_samples, test_prior_probs, test_labels, recording_frequency=1, lr=5e-3,
              weight_decay=5e-5):
        ...
        current_samples = torch.cat([train_samples, unlabeled_samples, test_samples])
        current_labels = torch.cat(
            [train_labels, unlabeled_prior_probs.unsqueeze(0).repeat(unlabeled_samples.shape[0], 1),test_prior_probs.unsqueeze(0).repeat(test_samples.shape[0], 1)])
...
for run in range(number_runs):
    ...
    structure = [[ConditionalRealNVPLayer, {'hidden_dims': [80, 80, 80]}] for i in range(3)] + [[ConditionalDIFLayer, {'hidden_dims': [32, 32], 'K': 3}] for i in range(1)]
    model_gen = GenerativeClassifierSemiSupervised(sample_dim,C, structure)
    model_gen.conditional_model.initialize_with_EM(torch.cat([train_samples, unlabeled_samples, test_samples], dim=0),50, verbose=False)
    aggregate_loss_trace,train_loss_trace, unlabeled_loss_trace, test_loss_trace, indices = [],[],[],[],[]
    for t in range(3):
        _ = model_gen.train(200, int(train_samples.shape[0] / 20), train_samples,train_prior_probs, train_labels, unlabeled_samples,unlabeled_prior_probs,unlabeled_labels, test_samples,test_prior_probs,test_labels, lr=5e-4, recording_frequency=10)
        aggregate_loss_trace +=_[0]
        train_loss_trace+=_[1]
        unlabeled_loss_trace+=_[2]
        test_loss_trace+=_[3]
    ...
    torch.save(unlabeled_loss_trace, "gen_MNIST_balanced/unlabeled_loss_trace_" + str(run) + ".pt")
