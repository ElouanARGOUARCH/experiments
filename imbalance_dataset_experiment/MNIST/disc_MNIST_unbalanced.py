...
class Classifier(torch.nn.Module):
    ...
    def train(self, epochs, batch_size,train_samples, train_labels, unlabeled_samples, unlabeled_labels,
              test_samples, test_labels, recording_frequency = 1, lr=5e-4, weight_decay=5e-5):
            ...
            if __ % recording_frequency == 0:
                with torch.no_grad():
                    ...
                    pbar.set_postfix_str('train_loss = ' + str(round(train_loss, 4)) + '; unlabeled_loss = ' + str(round(unlabeled_loss, 4)) + '; test_loss = ' + str(round(test_loss, 4)) + '; device = ' + str(
                        device))
        self.to(torch.device('cpu'))
        return train_loss_trace, unlabeled_loss_trace, test_loss_trace
...
for run in range(number_runs):
    ...
    train_loss_trace, unlabeled_loss_trace, test_loss_trace = model_disc.train(600,int(train_samples.shape[0]/20),train_samples,train_labels,unlabeled_samples,unlabeled_labels, test_samples, test_labels, recording_frequency=10)
    torch.save(train_loss_trace, "disc_MNIST_unbalanced/train_loss_trace_" + str(run) + ".pt")
    torch.save(unlabeled_loss_trace, "disc_MNIST_unbalanced/unlabeled_loss_trace_" + str(run) + ".pt")
    torch.save(test_loss_trace, "disc_MNIST_unbalanced/test_loss_trace_" + str(run) + ".pt")
