...
for run in range(number_runs):
    model_disc = torch.load("disc_MNIST_unbalanced/model_disc_" + str(run) +".pt")
    train_samples, train_prior_probs, train_labels,list_test_samples, list_test_prior_probs, list_test_labels,logit_transform, pca_transform = torch.load("disc_MNIST_unbalanced/datasets_" + str(run) + ".pt")
    print('run ' + str(run))
...
list_test_accuracy = []
for run in range(number_runs):
    model_gen = torch.load("gen_MNIST_unbalanced/model_gen_" + str(run) +".pt")
    train_samples, train_prior_probs, train_labels,test_samples, test_prior_probs, test_labels,logit_transform, pca_transform = torch.load("gen_MNIST_unbalanced/datasets_" + str(run) + ".pt")
    print('run ' + str(run))
